...
```{r load}
snf_data <- read_xlsx("SNF_CODED_CALCULATED_SCORES_04042022.xlsx",
...
snf_data <- snf_data %>%
  ...
  mutate(StayDuration = DischargeDate - CurrentAdmit,
         DtmDuration = DischargeDtm - AdmitDtm)

# create binary for readmission to hosp w/in 30 days
```  
...
```{r hist1}
score_piv <- snf_data %>% 
  dplyr::select(SternClientID, Year, CCI, LACE, HOSPITAL) %>%
  pivot_longer(!c(SternClientID, Year), names_to = "score", values_to = "val")
