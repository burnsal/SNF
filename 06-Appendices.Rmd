...
CCI_mod0_plots <- data.frame(CCI = seq(0, 14, 1),
...
cci_modfit <- pchisq(CCI_mod0$deviance, df=13, lower.tail = FALSE)
```

The results of the $\chi^2_{14}$ goodness-of-fit test (p = `r round(cci_modfit, 3)`) indicate evidence of lack of fit. This is likely due to the larger residuals for CCI values 13-14; could refit the model excluding this data which will result in better model fit, but it will not change the practical conclusion reached regarding the association between CCI score and risk of readmission.  
...
cci_nullmod <- glm(formula = cbind(n.Readmit, n - n.Readmit) ~ 1, 
                  family = binomial(link = "logit"), data = readmit_cci)

summary(cci_nullmod)

CCI_nullmod_plots <- data.frame(CCI = seq(0, 14, 1),
...
# Goodness-of-Fit Test
cci_nullfit <- pchisq(cci_nullmod$deviance, df=14, lower.tail = FALSE)
...
lace_mod0_plots <- data.frame(LACE = seq(4, 18, 1),
...
lace_mod0_gof <- pchisq(lace_mod0$deviance, df=13, lower.tail = FALSE)

# Base model without LACE=12
summary(lace_mod1)

lace_mod1_plots <- data.frame(LACE = c(seq(4, 11, 1), seq(13, 18, 1)),
...
lace_mod1_gof <- pchisq(lace_mod1$deviance, df=12, lower.tail = FALSE)
...
lace_nullmod_plots <- data.frame(LACE = seq(4, 18, 1),
...
# quickly look at null model with outlier removed
lace_out_null <- glm(formula = cbind(n.Readmit, n - n.Readmit) ~ 1, 
                  family = binomial(link="logit"), data = readmit_lace_cut)

# Goodness-of-Fit Test
lace_nullfit <- pchisq(lace_nullmod$deviance, df=14, lower.tail = FALSE)
